...
const CoverPunchase = ({ itemsPerPage }: { itemsPerPage: number }) => {
  const items = [...purchaseHistoryCover];
      ...
      <div className="flex flex-col justify-center items-center gap-4">
        ...
        <div className="flex gap-4 items-center justify-start flex-wrap">
          {currentItems.map((item, index) => (
            <div
              className="hover:opacity-70 transition-opacity flex justify-between items-center gap-4"
              key={index}
            >
              <Image
                src={item.src}
