...
function CarouselArticle({ limit, page }: { limit: number; page: number }) {
  const [articleCarousel, setArticleCarousel] = useState<Array<any>>([]);
  const [initComplete, setInitComplete] = useState<boolean>(false);
  const getArticleCarousel = () => {
    getLimitArticle(limit, page).then((response) => {
      setArticleCarousel(response?.data?.rows);
    });
  };
  useEffect(() => {
    getArticleCarousel();
      ...
      {articleCarousel?.length > 0 && (
        <div className="rounded-[20px] overflow-hidden">
          ...
          >
            ...
            >
              {articleCarousel?.map((item: any, index: number) => (
                <button
                  key={index}
                  type="button"
            ...
            {/* Carousel items */}
            <div className="relative w-full overflow-hidden after:clear-both after:block after:content-[''] rounded-[20px]">
              {articleCarousel?.map((item: any, index: number) => (
                <div
                  key={index}
                  className={`relative ${
                    index === 0 ? "" : "hidden"
                  } float-left -mr-[100%] w-full transition-transform duration-[600ms] ease-in-out motion-reduce:transition-none rounded-[10px]`}
                ...
                >
                  <div className="relative">
                    <div>
                      {item?.featured_image ? (
                        <Image
                          ...
                          height={1000}
                          alt="image"
                          className="w-full max-h-[537px] object-cover rounded-[5px]"
                        />
                      ) : (
                        <Image
                          priority={true}
                    ...
                    <div className="absolute bg-black/80 w-[380px] h-full top-0 right-0  flex flex-col justify-center items-start px-[36px] gap-2 z-30">
                    ...
                    Technology
                      ...
                      <p className="font-light text-xs leading-[16.8px] text-white text-truncate">
