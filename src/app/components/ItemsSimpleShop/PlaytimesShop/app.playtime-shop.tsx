...
const PlaytimesShop = ({
  ...
  const [idPlaytimesDetail, setIdPlaytimesDetail] = useState<number>();
  const handleModalBuyPlaytimesShop = async (id: number) => {
    setIsOpenModal(!isOpenModal);
    setIdPlaytimesDetail(id);
  };
  ...
  return (
    <>
      <div className="grid grid-cols-4 gap-4  animate-translateFadeIn transition-all">
        {currentItems.map((item, inx) => (
          <div key={inx} className="mt-[27px]">
            <div className="p-[10px] bg-pink-900 rounded-[30px] border border-pink-400">
              <div className="flex justify-center items-center w-full h-[204px] bg-[#340216] rounded-[20px]">
                <Image
                  priority={true}
                  src={palytimeDefault}
                  alt=""
              ...
              <div className="flex justify-end items-center gap-2 mt-2">
                ...
                >
                  {item?.user_inventory ? (
                    <button
                      onClick={() => toast.warning("Avatar already owned")}
                    ...
                    <button
                      onClick={() => handleModalBuyPlaytimesShop(item?.id)}
                      className="w-full h-full"
                    >
                      Buy
                    </button>
      ...
      <ModalBuy
        ...
        onClick={(id: number) => handleModalBuyPlaytimesShop(id)}
        id={idPlaytimesDetail}
      />
      <ReactPaginate
        breakLabel="..."
        nextLabel=">"
